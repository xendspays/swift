...
export default function WeChatQRPage() {
  ...
  const [description, setDescription] = useState('Payment via WeChat Pay');
  const [referenceId, setReferenceId] = useState('');
  const [showSuccessModal, setShowSuccessModal] = useState(false);
  const [generatedQR, setGeneratedQR] = useState<WeChatQRResponse | null>(null);
  ...
  const generateQRMutation = useMutation({
    mutationFn: async () => {
      ...
      const res = await client.apiCall.invoke({
        url: '/api/v1/magpie/wechat',
        method: 'POST',
        data: {
          amount: numAmount,
          description: description || 'WeChat Pay Payment',
    ...
    onSuccess: (data) => {
      ...
      toast.success('✅ WeChat Pay QR code generated successfully!');
    },
    onError: (error: any) => {
  ...
  const handleShare = () => {
    if (generatedQR?.checkout_url) {
      if (navigator.share) {
        navigator.share({
          title: 'WeChat Pay Payment',
          text: `Pay ₱${amount} via WeChat Pay - ${description}`,
      ...
      <SiteContainer className="py-8">
        ...
        <div className="mb-8 space-y-2">
          ...
          <div className="flex items-center gap-4">
            <div className="h-14 w-14 rounded-xl bg-gradient-to-br from-green-500/20 to-green-600/10 flex items-center justify-center">
              <Smartphone className="h-7 w-7 text-green-600" />
            </div>
            <div>
              <h1 className="text-4xl font-semibold tracking-tight text-foreground">WeChat Pay QR Code</h1>
              <p className="text-slate-500 font-medium">Generate a self-hosted QR code for WeChat Pay payments</p>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 space-y-6">
            <Card className="border-slate-200/80 bg-white shadow-sm">
              <CardHeader>
                <CardTitle>Create WeChat Pay Payment</CardTitle>
                <CardDescription>
                  Generate a QR code for WeChat Pay payment. Amount will be automatically converted from PHP to CNY.
                </CardDescription>
              </CardHeader>
              <CardContent className="space-y-6">
                ...
                <div className="space-y-3">
                  ...
                  <div className="relative">
                    ...
                    <Input
                      ...
                      className="pl-8 h-11 border-slate-200 focus:border-green-400"
                      step="0.01"
                      min="0"
                    />
                  </div>
                  <p className="text-xs text-slate-500">
                    Minimum: ₱50 | This will be converted to CNY for WeChat Pay processing
                  </p>
                </div>
                ...
                <div className="space-y-3">
                  ...
                  <Textarea
                    ...
                    className="h-24 border-slate-200 focus:border-green-400 resize-none"
                  />
                  <p className="text-xs text-slate-500">
                ...
                <div className="space-y-3">
                  ...
                  <Input
                    ...
                    className="h-11 border-slate-200 focus:border-green-400"
                  />
                  <p className="text-xs text-slate-500">
            ...
            <div className="grid grid-cols-2 gap-4">
              <Card className="border-slate-200/80 bg-gradient-to-br from-green-50 to-green-50/50">
                <CardContent className="pt-6">
                  <p className="text-2xl font-semibold text-green-700">{parseFloat(amount) || '0'}</p>
            ...
            {generatedQR ? (
              <Card className="border-slate-200/80 bg-white shadow-sm sticky top-24">
                ...
                <CardContent className="space-y-4">
                  ...
                  {/* Details */}
                  <div className="space-y-2 p-3 bg-green-50 rounded-lg border border-green-200/50">
                    <p className="text-xs font-semibold text-green-900 uppercase">Reference</p>
                    <p className="text-sm font-mono text-green-700 break-all">{generatedQR.reference_id}</p>
                  </div>
                ...
                <CardContent className="pt-6 flex flex-col items-center text-center space-y-4">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-slate-500 mt-1">
                      Fill in the form and click "Generate" to create your WeChat Pay QR code
                    </p>
                  </div>
                </CardContent>
              </Card>
        ...
        <DialogContent className="sm:max-w-[450px] bg-white rounded-2xl">
          ...
          <div className="space-y-4 py-4">
            ...
            <p className="text-xs text-slate-600 text-center">
              ✅ Your self-hosted WeChat Pay QR code is ready. Share the link above with customers to collect payments.
            </p>
          </div>
        </DialogContent>
      </Dialog>
